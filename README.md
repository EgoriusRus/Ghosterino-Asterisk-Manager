...
- 📁 Готовые конфиги Asterisk в `backend/results/`
...
Эта команда:
- Поднимет Docker Compose (PostgreSQL + API)
- Дождется готовности базы данных
- Заполнит базу тестовыми данными (seed)
- Сгенерирует конфиги Asterisk
...
make clean          # Полная очистка (контейнеры + volumes + конфиги)
